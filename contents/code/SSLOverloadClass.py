...
from Functions import readSocketReady, writeSocketReady, TIMEOUT, Path
...
class ThreadServer(ThreadingMixIn, SimpleXMLRPCServer):
	...
	def __init__(self, serveraddr, allow_none = False, TLS = False, certificatePath = ''):
		...
		if Path.platform == 'win' :
			self.socketInit(serveraddr, TLS, certificatePath = certificatePath)
		else :
			while count < 10 :
				self.socketInit(serveraddr, TLS, certificatePath = certificatePath)
	...
	def verify_request(self, request, client_address):
		...
		#if writeSocketReady(request, self.timeout) :
		#	return True
		#else : False
...
class SSLServerProxy(ServerProxy):
	...
	def socketInit(self, addr, TLS = False):
		if TLS :
			self.socket = ssl.wrap_socket(\
							socket.socket(socket.AF_INET, socket.SOCK_STREAM), \
							ca_certs = "/etc/ssl/ca_bundle.trust.crt", \
